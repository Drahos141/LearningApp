...
function makeQ() {
  ...
  else if (op === '-') { a = Math.floor(Math.random()*50)+1; b = Math.floor(Math.random()*a)+1; ans = a-b; }
...
export default function MathSprint() {
  ...
  useEffect(() => {
    ...
    const t = setInterval(() => setTime(t => {
      if (t <= 1) { clearInterval(t); setDone(true); return 0; }
      return t - 1;
    }), 1000);
  ...
  if (!started) return (
    <div style={{ textAlign: 'center' }}>
      ...
      <p style={{ marginBottom: '1.5rem', color: '#6b6b6b' }}>Solve as many arithmetic problems as you can in 60 seconds!</p>
  ...
  if (done) return (
    <div className="game-result">
      ...
      <button className="play-btn" style={{ marginTop: '1.5rem' }} onClick={() => { setScore(0); setTime(60); setDone(false); setStarted(false); setQ(makeQ()); setInput(''); }}>Play Again</button>
    </div>
  );
  ...
  return (
    <div>
      ...
      <div style={{ background: '#f8f8f8', border: '1px solid #e0e0e0', borderRadius: '12px', padding: '2.5rem', textAlign: 'center', marginBottom: '1.5rem' }}>
        <div style={{ fontSize: '2.5rem', fontWeight: 800 }}>{q.text} = ?</div>
      </div>
