    ...
    private static List<Category> BuildCategories()
    {
        ...
        var moreLessons = new Dictionary<int, List<(string Title, string Content)>>
        {
            { 1, new List<(string, string)>
            {
                ...
                ("Software Types and Applications", "Software is broadly divided into system software and application software. System software manages hardware and provides a foundation for other programs — it includes operating systems, device drivers, firmware, and utility tools such as antivirus, disk management, and backup programs.\n\nApplication software serves end-user needs: productivity suites (Microsoft Office, LibreOffice), web browsers, media players, games, and communication apps. Mobile apps, desktop apps, and web apps are all forms of application software.\n\nDistribution models matter for licensing. Commercial software requires purchase or subscription. Freeware is free to use but typically closed-source. Open-source software provides editable source code under licenses such as GPL or MIT. Shareware offers a trial before payment. SaaS (Software as a Service) delivers applications over the web on a subscription basis.\n\nKeeping software updated is essential for security. Unpatched vulnerabilities are among the leading causes of data breaches. Most operating systems and applications support automatic updates. Security patches should be applied promptly, especially for internet-facing software.\n\nChoosing the right software requires evaluating features, cost, security track record, performance impact, vendor support, and compatibility with existing systems. Free alternatives such as GIMP for image editing or LibreOffice for productivity often cover common use cases without the cost of commercial equivalents."),
                ("Data Representation Basics", "Computers store all information as binary — sequences of ones and zeros called bits. Eight bits form one byte. Data sizes scale from bytes to kilobytes (KB), megabytes (MB), gigabytes (GB), and terabytes (TB), each roughly 1,000 times the previous unit (1,024 in strict binary terms).\n\nText is represented by assigning numbers to characters. ASCII encoded 128 characters using 7 bits, sufficient for English but not global scripts. Unicode (with the UTF-8 encoding) assigns code points to over 140,000 characters across all writing systems worldwide, making international software possible with a single standard.\n\nImages are stored as grids of pixels, each defined by color values. RGB uses 8 bits per channel — 24 bits total — representing 16.7 million colors. Higher bit depths (10-bit, 12-bit) improve color gradients for professional photography and video.\n\nAudio is sampled thousands of times per second. CD quality uses 44,100 samples per second at 16 bits, producing high-fidelity sound. Higher sample rates and bit depths improve dynamic range for professional audio production.\n\nCompression reduces storage needs. Lossless formats (PNG, FLAC, ZIP) reconstruct data perfectly. Lossy formats (JPEG, MP3, H.264) discard data deemed imperceptible, achieving much smaller sizes at the cost of some quality. Understanding these trade-offs helps choose the right format — use lossless for archival, lossy for web delivery and media streaming."),
                ("Basic Computer Troubleshooting", "Troubleshooting is the process of systematically identifying, diagnosing, and resolving computer problems. A structured approach saves time and prevents accidental damage.\n\nThe basic troubleshooting process: (1) Identify — reproduce the problem, note error messages, gather information. (2) Theorize — consider the most likely causes starting with the simplest and most common. (3) Test — try one fix at a time to isolate which resolves the problem. (4) Implement — apply the confirmed fix. (5) Verify and document — confirm the fix works and record what was done.\n\nCommon issues: Computer won't start — check the power cable, outlet, and power button connection. Slow performance — use Task Manager (Windows) or Activity Monitor (macOS) to identify high CPU, RAM, or disk usage; scan for malware; free up disk space. No display — verify the monitor is connected to the GPU's video output, not the motherboard's integrated video port.\n\nBuilt-in diagnostic tools: Task Manager and Activity Monitor show running processes and resource usage. Device Manager flags hardware driver issues with warning icons. Event Viewer logs system errors and crash details. System Restore lets Windows roll back to a previous working state.\n\nSafe Mode starts the OS with minimal drivers to isolate software conflicts — if the problem disappears in Safe Mode, a third-party driver or application is the likely cause. When hardware physically fails or data is at risk, professional repair is the safest option. Always back up important data before undertaking major troubleshooting steps.")
            }},
            ...
            { 6, new List<(string, string)>
            {
                ...
                ("Python Functions In Depth", "Functions are named, reusable blocks of code that accept inputs, perform a task, and optionally return outputs. Well-designed functions have a single responsibility, a clear name, and meaningful documentation.\n\nDefining a function: use the def keyword followed by the function name, parentheses for parameters, and a colon. The function body is indented. Parameters are placeholders; arguments are the actual values passed at call time. The return statement sends a value back to the caller. Without an explicit return, a function returns None.\n\nDefault parameters make arguments optional: def greet(name, greeting='Hello'): return f'{greeting}, {name}!'. *args captures any number of positional arguments as a tuple; **kwargs captures keyword arguments as a dictionary. These enable flexible, variadic function interfaces.\n\nScope: variables created inside a function are local and invisible outside. Global variables can be read inside functions, but modifying them requires the global keyword — use sparingly, as global state complicates testing and debugging.\n\nLambda expressions create anonymous one-liner functions: square = lambda x: x ** 2. They are most useful as arguments to functions like sorted(list, key=lambda x: x['age']) and map(). For anything more complex than a single expression, a named function with a docstring is clearer and easier to maintain."),
                ("Python Collections In Depth", "Python's built-in collection types — list, tuple, set, and dictionary — are the workhorses of data manipulation. Choosing the right type for the task improves both code clarity and performance.\n\nLists are ordered, mutable sequences. Key operations: append(item) adds to the end, insert(i, item) adds at a position, remove(value) deletes the first match, pop(i) removes and returns by index. sorted() returns a new sorted list; .sort() sorts in-place. List comprehensions compactly build filtered or transformed lists: squares = [x**2 for x in range(10) if x % 2 == 0].\n\nDictionaries store key-value pairs with fast O(1) lookup. Access: d[key] raises KeyError if missing, or d.get(key, default) for safe access. Iterate over pairs with d.items(). Dict comprehensions: {k: v*2 for k, v in data.items()}. Python 3.7+ guarantees insertion order.\n\nSets store unique values with no ordering, providing O(1) membership testing. Ideal for deduplication (set(my_list)) and set operations: union (|), intersection (&), difference (-). Frozensets are immutable sets that can be used as dictionary keys.\n\nTuples are ordered and immutable. They signal that the data should not change — coordinates, RGB values, function return pairs. Named tuples (from collections) add attribute access to tuple elements, improving readability without the overhead of a full class."),
                ("String Manipulation in Python", "Python strings are immutable sequences of Unicode characters with a comprehensive set of built-in methods for searching, transforming, and formatting text.\n\nEssential string methods: upper() / lower() for case conversion; strip() removes whitespace from both ends; split(sep) breaks a string into a list; ', '.join(list) assembles a list into a string; replace(old, new) substitutes substrings; find(sub) returns the index of the first match or -1; startswith() / endswith() test string boundaries.\n\nF-strings (formatted string literals) are the modern, readable way to embed expressions: name = 'Alice'; msg = f'Hello, {name}! Result: {2+2}'. They support format specifiers: f'{price:.2f}' for two decimal places, f'{num:05d}' for zero-padded integers. Older .format() and % formatting still appear in legacy code.\n\nString slicing uses [start:stop:step] syntax. s[0:5] extracts the first five characters. s[-3:] returns the last three. s[::-1] reverses the string. Strings are zero-indexed; negative indices count from the end. Since strings are immutable, all operations return new strings.\n\nThe re module provides regular expressions for complex pattern matching: re.search(pattern, string) finds any match; re.findall returns all matches; re.sub replaces matches. Key syntax: . (any char), * (zero or more), + (one or more), [abc] (character class), ^ (start), $ (end), \\d (digit), \\w (word character). Regular expressions are invaluable for data cleaning, log parsing, and validation."),
                ("Decorators and Generators", "Decorators and generators are advanced Python features that make code more concise, expressive, and memory-efficient.\n\nA decorator wraps a function to modify or extend its behavior without changing its source code. Defined using the @decorator syntax, a decorator is a higher-order function: it takes a function as input and returns a new function. Common uses include logging execution time, enforcing authentication, caching results (memoization), and validating inputs. Always apply @functools.wraps to preserve the wrapped function's metadata such as its name and docstring.\n\nGenerators produce values lazily using the yield keyword. Where a regular function computes and returns all results at once, a generator function returns a generator object that produces one value at a time on demand. This is ideal for processing large files, infinite sequences, and data streams without loading everything into memory. range() in Python 3 is generator-like. Generator expressions use parentheses: total = sum(x**2 for x in data).\n\nThe itertools module provides powerful tools built on generators: chain() flattens multiple iterables, cycle() repeats a sequence, islice() slices a lazy sequence, and product() / combinations() / permutations() generate combinatorial sequences efficiently.\n\nDecorators and generators represent Python's capacity for clean abstraction. Mastering them allows you to write professional-grade code that is more composable, testable, and efficient than equivalent imperative alternatives."),
                ("Virtual Environments and Project Structure", "Virtual environments isolate Python dependencies per project, preventing version conflicts between projects sharing the same machine.\n\nCreate a virtual environment: python -m venv venv. Activate with source venv/bin/activate (macOS/Linux) or venv\\Scripts\\activate (Windows). The prompt prefix confirms activation. Packages installed with pip only affect this environment. Deactivate with the deactivate command. Always add venv/ to .gitignore — environments are not committed to version control.\n\nA conventional Python project layout: project_root/ contains src/package_name/ or a top-level package directory for source code, tests/ for test modules, requirements.txt for dependencies, README.md for documentation, and .gitignore. Sensitive values like API keys belong in a .env file loaded with python-dotenv — never committed. requirements.txt is generated with pip freeze > requirements.txt and reproduced on any machine with pip install -r requirements.txt.\n\nFor larger or publishable projects, tools like Poetry or Hatch manage dependencies, versioning, and packaging in a single workflow. pyproject.toml is the modern configuration standard replacing setup.py.\n\nConsistent project structure reduces onboarding time, makes automation straightforward, and signals professionalism to collaborators. Following community conventions means new contributors instantly know where tests live, how to install dependencies, and where configuration belongs."),
                ("Comprehensions and Functional Tools", "Python comprehensions and built-in functional tools allow concise, expressive data transformations that are often clearer and more efficient than equivalent loops.\n\nList comprehensions build new lists: squares = [x**2 for x in range(10)]; evens = [x for x in data if x % 2 == 0]. They run faster than equivalent for loops because iteration is handled in C internally. Use them when creating a new list from an existing iterable is the primary goal — avoid complex nested comprehensions that reduce readability.\n\nDictionary and set comprehensions follow the same syntax: {k: v.upper() for k, v in mapping.items()} builds a transformed dict; {x % 5 for x in data} builds a set of unique remainders. Generator expressions use parentheses and produce values lazily: total = sum(x**2 for x in range(10**6)) avoids allocating a large intermediate list.\n\nBuilt-in higher-order functions: map(fn, iterable) applies fn to every element lazily; filter(pred, iterable) keeps elements matching the predicate; sorted(iterable, key=fn) sorts by a key function; zip(a, b) pairs two iterables element-wise. functools.reduce() accumulates a sequence using a binary operation. functools.partial() creates a new function with arguments pre-filled, useful for callback patterns.\n\nFunctional tools encourage a pipeline style where data flows through small, composable transformations. Paired with generator expressions, they enable memory-efficient processing of large datasets without intermediate lists occupying memory for their full duration.")
            }},
            { 7, new List<(string, string)>
            {
                ...
                ("TypeScript Basics", "TypeScript is a statically typed superset of JavaScript that compiles to plain JavaScript. Developed by Microsoft, it adds optional type annotations that catch errors at compile time rather than at runtime, improving code quality and developer experience.\n\nType annotations: const name: string = 'Alice'; let count: number = 42; function greet(name: string): string { return `Hello ${name}`; }. TypeScript infers types from context automatically. Interfaces define object shapes: interface User { id: number; name: string; email?: string; }. The ? marks optional properties. Type aliases: type Status = 'active' | 'inactive'.\n\nUnion types allow a variable to hold multiple types: let id: string | number. Generics create reusable typed components: function identity<T>(value: T): T { return value; }. Enums define named constants: enum Direction { Up, Down, Left, Right }. Type assertions: (event.target as HTMLInputElement).value.\n\ntsconfig.json configures the TypeScript compiler. Enabling strict mode activates all strict type checks — the recommended setting for new projects. TypeScript integrates with VS Code, WebStorm, and all major editors for autocompletion, inline errors, and safe renaming.\n\nTypeScript adoption in large codebases is driven by developer experience: autocompletion across module boundaries, refactoring confidence, and self-documenting APIs that reduce the need for separate documentation. Most modern JavaScript frameworks and libraries ship TypeScript definitions."),
                ("React Fundamentals", "React is a JavaScript library for building user interfaces through a component-based architecture. Each component is a reusable function that returns JSX — an HTML-like syntax embedded in JavaScript that describes the UI.\n\nComponents receive immutable data via props and manage local state with useState: function Counter() { const [count, setCount] = useState(0); return <button onClick={() => setCount(count + 1)}>Count: {count}</button>; }. Each state update triggers a re-render with the updated value. Derived values should be computed directly from state — avoid duplicating state.\n\nuseEffect handles side effects: data fetching, subscriptions, and timers. useEffect(() => { fetchData().then(setData); }, [dependency]). The dependency array controls when the effect runs: empty array means once on mount; including a variable means on every change of that variable. Return a cleanup function to cancel subscriptions and timers.\n\nComponent composition builds complex UIs from small, focused pieces. Pass data down via props; communicate events up via callback props. Lifting state up to a common parent enables sibling components to share data. Context avoids prop-drilling for global values like theme or auth.\n\nReact Developer Tools (browser extension) visualizes the component tree, current props, and state for debugging. Vite and Create React App are popular project starters. Understanding React's unidirectional data flow — state flows down, events bubble up — makes applications predictable and easy to test at scale."),
                ("Web Security Fundamentals", "Web security protects users and server resources from malicious actors. Understanding common vulnerabilities and their mitigations is essential for every web developer.\n\nCross-Site Scripting (XSS) injects malicious scripts into pages viewed by other users. Reflected XSS comes from URL parameters; stored XSS is persisted in a database. Prevention: always escape or encode user-supplied content before rendering it in HTML. Content Security Policy (CSP) headers limit which scripts the browser will execute, providing a second layer of defense.\n\nCross-Site Request Forgery (CSRF) tricks a logged-in user's browser into making unintended requests to your server. Defense: use CSRF tokens (server generates a token, form submits it, server validates it) and SameSite cookie attribute (Lax or Strict) to prevent cross-origin cookie transmission.\n\nSQL Injection inserts malicious SQL into query inputs. Always use parameterized queries or prepared statements: never concatenate user input into SQL strings. An ORM typically handles this automatically. Input validation and least-privilege database accounts reduce impact.\n\nAdditional practices: serve all content over HTTPS and redirect HTTP, set the Secure and HttpOnly flags on cookies, use Helmet.js (Node) or equivalent middleware to set security headers, keep dependencies updated with automated scanning tools, implement rate limiting to resist brute-force attacks, and store passwords using bcrypt or Argon2 — never plain text or reversible encryption. A security-first mindset catches vulnerabilities during development rather than after deployment.")
            }},
            { 11, new List<(string, string)>
            {
                ...
                ("Subject-Verb Agreement", "Subject-verb agreement requires that a verb match its subject in number (singular or plural). Most errors occur with complex structures, collective nouns, or certain pronouns rather than straightforward cases.\n\nBasic rule: singular subjects take singular verbs; plural subjects take plural verbs. 'The cat sits.' (singular) 'The cats sit.' (plural). Third-person singular verbs add -s or -es: walks, watches, runs.\n\nCollective nouns (team, class, committee, staff, jury) present a common difficulty. In American English, they typically take singular verbs: 'The team has decided.' In British English, plural verbs are often preferred: 'The team have decided.' Choose one convention and apply it consistently throughout a document.\n\nIndefinite pronouns follow specific rules. Always singular: everyone, somebody, nobody, either, neither, each. Always plural: both, few, many, several. Context-dependent: all, some, most, none — 'All of the water is gone.' (uncountable) vs. 'All of the students are ready.' (countable).\n\nWith compound subjects joined by 'and', use a plural verb: 'Tom and Lisa are partners.' With 'or' or 'nor', match the verb to the nearest subject: 'Neither the manager nor the employees are responsible.' Inverted sentences (There is/are) require identifying the true subject that follows: 'There are three solutions.' Regular practice with editing real sentences builds the intuitive sense needed for natural agreement."),
                ("Punctuation Essentials", "Punctuation marks guide readers through sentence structure, pace, and meaning. Correct use prevents ambiguity and signals the relationship between ideas.\n\nCommas have multiple uses: after an introductory phrase ('In summary, the results were positive'); to separate items in a series ('apples, oranges, and bananas' — the Oxford comma before 'and' prevents ambiguity); before a coordinating conjunction joining two independent clauses ('I was tired, but I stayed'); and around non-essential information ('My colleague, who joined last month, is leading the project').\n\nThe semicolon connects two closely related independent clauses without a conjunction: 'The deadline was Friday; the report was unfinished.' It also separates list items that themselves contain commas: 'The panel included Dr. Lee, a cardiologist; Ms. Carter, a nutritionist; and Dr. Hall, a physiologist.'\n\nThe colon introduces what follows — a list, explanation, or quotation — after a complete independent clause: 'There are three rules: be present, be honest, and be kind.' Apostrophes show possession (the company's policy) and form contractions (it's = it is; don't = do not). The possessive pronoun 'its' has no apostrophe — only the contraction 'it's' does.\n\nExclamation marks add strong emphasis and should be used sparingly in professional writing. Question marks are straightforward but note that indirect questions do not use them: 'She asked where he was going.' Mastering punctuation lifts the clarity and professionalism of all written communication."),
                ("Common Grammar and Usage Errors", "Many grammar errors are predictable and correctable with focused attention. Recognizing frequent mistakes builds confidence in both speaking and writing.\n\nCommonly confused word pairs: their (possessive), there (location), they're (they are); your (possessive) vs. you're (you are); its (possessive) vs. it's (it is); affect (usually a verb) vs. effect (usually a noun); then (time) vs. than (comparison); who's (who is) vs. whose (possessive).\n\nDangling modifiers occur when a participial phrase does not clearly attach to the sentence's grammatical subject. 'Arriving late, the meeting had already started' implies the meeting arrived late. Correction: 'Arriving late, I found the meeting had already started.' The subject of the main clause must match the implied subject of the opening phrase.\n\nRun-on sentences incorrectly merge two independent clauses: 'I love coffee it gives me energy.' Fixes: use a period (two sentences); add a semicolon; add a coordinating conjunction with a comma; or use a subordinating conjunction. Sentence fragments lack a subject, a verb, or both — fix by connecting to the main sentence or adding the missing element.\n\nParallelism requires coordinated elements to use the same grammatical form. 'She enjoys hiking, to swim, and reading' mixes forms. Correct: 'She enjoys hiking, swimming, and reading.' Apply parallel structure to lists, headings, and paired comparisons for clarity and professional polish.")
            }},
            ...
            { 16, new List<(string, string)>
            {
                ...
                ("Network Protocols Overview", "A network protocol is a set of agreed-upon rules that defines how data is formatted, transmitted, received, and acknowledged between devices. Without standard protocols, devices from different manufacturers could not communicate — protocols make the internet and enterprise networks possible.\n\nProtocols operate at specific layers of the network model. Application layer protocols serve user-facing communication needs: HTTP and HTTPS transfer web pages, DNS resolves names to IP addresses, SMTP sends email, FTP transfers files, and DHCP automatically assigns IP addresses. Transport layer protocols (TCP, UDP) control how data is segmented and delivered. Network layer protocols (IP) handle logical addressing and routing between networks.\n\nThe IETF (Internet Engineering Task Force) and IEEE (Institute of Electrical and Electronics Engineers) publish open standards as RFCs and IEEE specifications. Open standards ensure interoperability — your browser from any manufacturer can communicate with any compliant web server. Proprietary protocols require compatible equipment on both ends.\n\nProtocols are layered — each layer builds on services from the layer below and provides services to the layer above. Application protocols do not need to know about physical cables; they rely on transport and network layers to deliver data reliably and correctly addressed.\n\nUnderstanding which protocol is responsible for which function is essential for troubleshooting. Connectivity loss may indicate an IP routing issue; name resolution failures point to DNS; authentication failures may involve LDAP or RADIUS. Knowing protocols makes diagnosis far more efficient than blind trial-and-error."),
                ("Default Gateways and Routing", "Every networked device must decide where to send traffic destined for IP addresses outside its local subnet. The default gateway is the router address the device forwards this traffic to — the 'exit door' connecting the local network to other networks and the internet.\n\nWhen a host sends a packet, it first checks whether the destination IP is in the same subnet. If yes, it delivers directly using ARP to find the destination's MAC address. If no, it forwards the packet to the default gateway. A missing or incorrect default gateway setting is a common cause of being unable to reach the internet while still communicating locally.\n\nA routing table is a database on each router listing paths to known networks. Each entry contains the destination network prefix, the subnet mask, the next-hop IP address or outgoing interface, and a metric. The router matches incoming packets to the most specific (longest prefix) matching entry.\n\nStatic routes are manually configured — simple and predictable, but do not adapt to topology changes. Dynamic routing protocols (OSPF, BGP, EIGRP) automatically exchange routing information between routers and recalculate paths when links fail or new routes become available. BGP routes traffic between autonomous systems across the global internet.\n\nThe default route (0.0.0.0/0) is the catch-all: if no more specific match exists, the packet goes to the default gateway. Home and small office routers maintain a single default route pointing to the ISP. Diagnosing gateway issues: 'route print' (Windows) or 'ip route show' (Linux) displays the routing table, revealing missing or incorrect routes."),
                ("Network Performance Basics", "Network performance affects user experience, application reliability, and productivity. Understanding the key metrics helps identify bottlenecks, plan capacity, and diagnose problems effectively.\n\nBandwidth is the maximum data rate of a link, measured in Mbps or Gbps. Higher bandwidth means more data can flow simultaneously — often described as 'pipe width.' Home internet connections are typically 100 Mbps to 1 Gbps; enterprise backbone links may reach 100 Gbps or beyond.\n\nLatency (delay or ping) is the time for a packet to travel from source to destination and back, measured in milliseconds. It is affected by physical distance, number of router hops, and processing delays. High latency makes real-time applications like VoIP, video conferencing, and gaming choppy and unresponsive even when bandwidth is adequate.\n\nThroughput is the actual data rate achieved in practice — almost always lower than theoretical bandwidth due to protocol overhead, congestion, hardware limits, and error recovery. Jitter is variation in latency between packets; high jitter degrades streaming audio and video quality even when average latency is acceptable.\n\nPacket loss occurs when packets fail to arrive. TCP detects loss and retransmits, reducing throughput but ensuring reliability. UDP does not retransmit — even small amounts of loss noticeably degrade audio and video quality. Causes include congestion, faulty cables, and failing hardware.\n\nTools for measuring: ping tests reachability, latency, and packet loss; iperf3 measures throughput between two hosts; Wireshark and tcpdump capture packets for detailed analysis. Regular performance baselining makes deviations and degradation much easier to identify and diagnose.")
            }},
            { 17, new List<(string, string)>
            {
            ...
            { 21, new List<(string, string)>
            {
                ...
                ("Data Collection and Preparation", "Data quality is the single biggest factor in machine learning outcomes. Models can only learn patterns that exist in training data — if data is biased, incomplete, or mislabeled, the model inherits those flaws regardless of algorithm sophistication.\n\nData sources vary by domain: relational databases, CSV files, public datasets (Kaggle, UCI ML Repository, Hugging Face), APIs, web scraping, sensor streams, and human labeling pipelines. Data must be representative of the population the model will encounter in deployment — distribution shift between training and real-world data is a primary cause of production failures.\n\nHandling missing values: drop rows or columns with excessive missingness; impute with mean, median, or mode for numerical data; use domain knowledge or a dedicated 'missing' category for categoricals; or use model-based imputation for complex cases. Documenting the rationale prevents introducing hidden biases.\n\nData cleaning: remove duplicates, standardize inconsistent formats (dates, capitalization, units), correct typos in categorical fields, and investigate outliers before deciding whether to remove or keep them. Outliers sometimes signal the most important phenomena — fraud, equipment failure, rare medical conditions.\n\nData labeling is often the bottleneck in supervised learning. Quality control, clear labeling guidelines, and inter-annotator agreement measurement keep labeled datasets reliable. Tools like Label Studio and Scale AI support annotation workflows. Active learning selects the most informative unlabeled examples for human review, maximizing labeling efficiency."),
                ("Cross-Validation and Model Selection", "Selecting the best model and estimating its real-world performance requires careful experimental design. Cross-validation is the standard approach when data is limited.\n\nA simple train/test split (typically 80/20) evaluates a model on a single held-out portion. Results can vary significantly based on which observations end up in the test set, especially with small datasets. Cross-validation addresses this by repeating the split multiple times.\n\nK-fold cross-validation divides data into k non-overlapping folds. The model trains on k-1 folds and evaluates on the remaining fold, rotating through all k combinations. Averaging the k results provides a more reliable performance estimate. k=5 and k=10 are standard choices. Stratified k-fold preserves class proportions in each fold — essential for imbalanced datasets.\n\nHyperparameter tuning searches for the configuration that maximizes validation performance. Grid search exhaustively tests all combinations in a parameter grid. Randomized search samples combinations randomly — more efficient for large grids. Bayesian optimization uses previous results to focus on promising parameter regions.\n\nThe test set must remain untouched until the very end — using it for any decision during development leads to optimistic performance estimates that fail in production. After all decisions are made, reporting performance on the held-out test set provides a realistic estimate of how the model will generalize to new data."),
                ("Introduction to NLP and Computer Vision", "Natural Language Processing (NLP) and Computer Vision (CV) are the two most widely deployed ML application areas, enabling computers to understand text and images respectively.\n\nCore NLP tasks include text classification (spam detection, sentiment analysis), named entity recognition, machine translation, question answering, and text generation. Preprocessing steps include tokenization, lowercasing, removing stop words, and normalizing word forms via stemming or lemmatization.\n\nNLP representation has evolved from simple word counts (bag-of-words, TF-IDF) to dense word embeddings (Word2Vec, GloVe) and contextual Transformer embeddings (BERT, GPT). Transformers capture long-range dependencies and contextual meaning, dramatically improving performance. Large Language Models (LLMs) extend this to conversational AI, code generation, and creative writing.\n\nCore CV tasks include image classification (what object is in this image?), object detection (where are objects?), semantic segmentation (classify each pixel), and image generation. CNNs were the dominant architecture for a decade; Vision Transformers (ViT) now match or exceed CNN performance on many benchmarks.\n\nTransfer learning is essential in both domains. Fine-tuning a model pretrained on large datasets dramatically outperforms training from scratch, especially with limited labeled data. Hugging Face Transformers provides ready-to-use pretrained models for NLP; torchvision and TensorFlow Hub offer similar resources for computer vision. Both fields are rapidly evolving, with multimodal models processing text and images together.")
            }},
            { 22, new List<(string, string)>
            {
            ...
            { 26, new List<(string, string)>
            {
                ...
                ("Digital Communication and Remote Work", "The rise of distributed work has made digital communication proficiency as important as in-person skills. Remote teams depend on intentional, clear, and respectful digital interactions to maintain alignment and productivity.\n\nSynchronous communication happens in real time — video calls, phone calls, instant messaging — and is best for nuanced discussions, decisions, relationship building, and brainstorming. Asynchronous communication allows responses at different times — email, written updates, recorded videos, project management tools — and is better for sharing information, requesting non-urgent input, and tasks that don't require immediate back-and-forth.\n\nVideo call effectiveness: test audio and video before joining; use a professional background; mute when not speaking; look at the camera rather than your own image; signal engagement through head nods and brief verbal affirmations. Start meetings with a clear agenda and expected outcomes. Avoid multitasking — it is visible and signals disrespect.\n\nInstant messaging etiquette: keep messages concise and purposeful; use threads to organize conversations; set your status to communicate availability; avoid expecting instant responses outside agreed working hours. Over-reliance on synchronous chat creates constant-interruption culture that reduces deep work time.\n\nWriting quality matters more in remote environments because text lacks tone and body language cues. Provide context upfront, be explicit about deadlines and next steps, and default to documenting decisions in writing so remote colleagues have equal access to information. Thoughtful async communication reduces unnecessary meetings and creates a searchable record that benefits the whole team."),
                ("Storytelling in Professional Contexts", "Storytelling is one of the most powerful tools for persuading, teaching, and inspiring. In business pitches, team meetings, executive presentations, and one-on-one conversations, a well-told story is far more memorable than abstract data or logic alone.\n\nThe fundamental story arc: situation (context and characters), complication (the problem or tension), turning point (the decision or insight), and resolution (the outcome and its implications). This arc creates tension and release that holds attention and makes information memorable.\n\nBusiness storytelling applications: making a compelling case for resources ('Here's the customer problem, what we learned trying to solve it, and why this investment will succeed'); presenting data findings ('The numbers tell a specific story — let me walk you through what changed and why it matters'); building culture with origin stories; and customer-facing narratives connecting pain points to solutions.\n\nData storytelling combines numbers with narrative. Lead with the insight or recommendation rather than the data; build supporting evidence progressively; use visuals to reinforce one point each rather than creating data dumps. The best data stories answer 'so what?' — why does this number matter, and what should we do about it?\n\nAuthenticity and specificity make stories persuasive. Use real names, concrete details, and genuine stakes where appropriate. Brevity is essential — a well-told two-minute story often has more impact than a ten-minute presentation. Practice telling key professional stories (a project you're proud of, a problem you solved, a lesson you learned) until they flow naturally."),
                ("Professional Networking and Relationship Building", "Professional relationships are the foundation of career growth, knowledge access, and opportunity creation. Effective networking is not about collecting contacts — it is about building genuine, mutually beneficial relationships over time.\n\nNetworking mindset: the most effective approach is curiosity-first and value-first. Rather than asking 'what can I get?', ask 'what can I offer?' — introductions, knowledge, feedback, or encouragement. Long-term reciprocity builds the trust that makes people want to help when opportunities arise.\n\nBuilding relationships: industry events, conferences, and professional meetups create structured opportunities to meet peers outside your organization. Prepare thoughtful questions about the other person's work. Follow up within 48 hours — a specific reference to your conversation turns a brief meeting into the beginning of a professional relationship. Internal networking across departments and levels is equally valuable and often overlooked.\n\nDigital presence: a maintained LinkedIn profile with clear experience, skills, and genuine recommendations makes you findable and credible. Regular, thoughtful engagement — commenting on industry discussions with specific insight, congratulating peers on achievements — builds visibility without self-promotion. Posting original observations shows expertise and keeps you top of mind in your field.\n\nMaintaining relationships requires consistent small investments: sharing a relevant article, celebrating a colleague's milestone, making a useful introduction. A small number of deep, trusted relationships in your field are often more valuable than many superficial connections. Strong professional networks provide access to informal knowledge, honest feedback, and opportunities that never appear on job boards.")
            }},
            { 27, new List<(string, string)>
            {
            ...
            { 31, new List<(string, string)>
            {
                ...
                ("Expansion Cards and PCIe", "PCIe (Peripheral Component Interconnect Express) is the standard high-speed interface connecting expansion cards to the motherboard and CPU. It uses serial lanes — each lane is a bidirectional connection with speeds of approximately 1 GB/s per lane on PCIe 3.0, 2 GB/s on PCIe 4.0, and 4 GB/s on PCIe 5.0.\n\nSlot sizes correspond to the number of lanes: x1 (1 lane), x4 (4 lanes), x8 (8 lanes), x16 (16 lanes). Discrete GPUs typically use the x16 slot for maximum bandwidth. PCIe is backward and forward compatible — a Gen 4 card in a Gen 3 slot operates at Gen 3 speeds without damage to either component.\n\nCommon expansion cards: discrete GPUs (gaming, machine learning, CAD workloads); NVMe SSDs in M.2 PCIe slots (far faster than SATA — up to 7,000 MB/s on Gen 4 vs 550 MB/s on SATA); 2.5G/10G Ethernet NICs for faster networking; Wi-Fi/Bluetooth cards for wireless desktop connectivity; capture cards for recording HDMI input; and professional audio interfaces.\n\nWhen planning expansion: verify the motherboard has available slots of the needed size; confirm the case has enough physical clearance (multi-slot GPUs can occupy 3+ expansion slot spaces); check that the PSU can supply sufficient wattage; and determine whether CPU lanes or chipset lanes are used (CPU lanes offer lower latency).\n\nFor GPU-accelerated workloads such as machine learning, PCIe bandwidth rarely bottlenecks modern workflows, but for technologies like DirectStorage (direct GPU-to-storage data transfer), higher PCIe generations increasingly matter for peak performance."),
                ("Computer Ports and Connectivity", "Understanding computer ports and connectivity standards helps you select the right cables and peripherals, diagnose compatibility issues, and future-proof your setup.\n\nUSB (Universal Serial Bus) is the dominant peripheral interface. USB Type-A is the flat rectangular port; Type-C is the small, oval, reversible connector. Speed tiers: USB 2.0 (480 Mbps — sufficient for keyboards, mice, webcams), USB 3.2 Gen 1 (5 Gbps, blue ports), USB 3.2 Gen 2 (10 Gbps), USB 3.2 Gen 2x2 (20 Gbps), USB4 Gen 3 (40 Gbps). USB Power Delivery can charge laptops at up to 240W over Type-C.\n\nThunderbolt (Intel, using the USB-C connector) provides higher performance than USB: Thunderbolt 4 at 40 Gbps supports daisy-chaining up to six devices, dual 4K displays, and 100W power delivery over a single cable. Thunderbolt 5 extends bandwidth to 120 Gbps for demanding professional workloads.\n\nDisplay connections: HDMI is ubiquitous on consumer devices — HDMI 2.1 supports 4K at 144 Hz and 8K at 60 Hz. DisplayPort offers higher bandwidth (DisplayPort 2.1 supports 8K and beyond) and is standard on monitors and GPUs. Both carry audio alongside video. USB-C with DisplayPort Alt Mode or HDMI Alt Mode connects displays over USB-C without a dedicated video output.\n\nOther common ports: 3.5 mm audio jack (headphones and microphones), SD card slot (cameras), Ethernet RJ-45 (wired networking — typically 1 Gbps, newer systems 2.5 Gbps), and legacy ports (VGA, DVI) found on older monitors and projectors. Identifying port types by shape and colour prevents forced insertions that damage connectors."),
                ("Choosing the Right PC Components", "Building or upgrading a PC requires matching components to your specific workload, budget, and future needs. Understanding how components interact prevents bottlenecks and wasted spending on capability you cannot use.\n\nWorkload-driven selection: gaming prioritizes GPU performance and fast NVMe storage; video editing and 3D rendering need a powerful multi-core CPU plus plenty of RAM (32–128 GB); general office productivity runs well on mid-range CPUs with 16 GB RAM; machine learning workloads benefit from VRAM-rich GPUs (24 GB+); servers prioritize reliability, ECC memory, and redundant storage over peak single-core speed.\n\nCPU selection: verify socket compatibility with the motherboard chipset. Compare cores and threads for parallelizable workloads (rendering, compilation, virtualization) and single-core speed for gaming and interactive tasks. AMD Ryzen and Intel Core offer competitive products at every price tier — compare real benchmark data for your target applications rather than raw spec sheet numbers.\n\nRAM: install at least 16 GB for modern desktop use; 32 GB for content creation. Match the DDR generation (DDR4 or DDR5) to the motherboard. Install in the recommended dual-channel slots (check the manual). Enable XMP (Intel) or EXPO (AMD) in BIOS to run at the rated speed rather than the slower default.\n\nStorage strategy: use a NVMe SSD for the operating system and active projects (fast load times); add a large HDD or secondary SSD for bulk storage. For the PSU, calculate component TDP, add 20–25% headroom, and choose a reputable brand with an 80 PLUS efficiency rating. An undersized or low-quality PSU can cause instability and risks hardware damage under full load.")
            }},
            { 32, new List<(string, string)>
            {
            ...
            {
                ("Arrays and Linked Lists", "Arrays and linked lists are the two fundamental sequential data structures. Understanding their trade-offs is the starting point for every data structures course and technical interview.\n\nArrays store elements in contiguous memory, enabling O(1) random access by index. Appending to the end of a dynamic array is amortized O(1) — the array doubles its capacity when full, so occasional O(n) copies are rare. Inserting or removing in the middle requires shifting all subsequent elements — O(n). Sequential memory layout is cache-friendly: modern CPUs pre-fetch contiguous blocks, making array iteration very fast in practice. Python lists, JavaScript Arrays, and Java ArrayLists are all dynamic array implementations.\n\nLinked lists store elements as nodes where each node holds data and a pointer to the next node. Singly linked lists navigate forward only; doubly linked lists have pointers in both directions, enabling efficient reverse traversal and tail insertions. Insertion and deletion at a known node are O(1) — just rewire pointers. Random access requires traversal from the head — O(n). Linked lists have no pre-allocation requirement and can grow without copying.\n\nPractical choice: use arrays when random access and cache performance matter. Use linked lists when frequent insertions or deletions occur at arbitrary positions and random access is rarely needed. Most high-level languages default to dynamic arrays — linked lists appear in LRU caches, undo systems, and OS scheduling queues.\n\nBig-O notation quantifies how execution time scales with input size. O(1) is constant, O(log n) is logarithmic, O(n) is linear, O(n log n) is linearithmic, O(n²) is quadratic. Analysing complexity before coding prevents building solutions that become unusable at scale."),
                ("Stacks, Queues, and Hash Tables", "Stacks, queues, and hash tables are abstract data types with specific access patterns that solve distinct classes of problems efficiently and appear throughout real software systems.\n\nA stack is a LIFO (Last In, First Out) structure: push adds to the top, pop removes from the top, peek inspects without removing. Natural applications: the call stack (function invocations), undo/redo operations, expression evaluation, matching balanced parentheses, and depth-first search. Python lists support stack operations with append() and pop(). Languages use a hardware call stack for function frames at the CPU level.\n\nA queue is FIFO (First In, First Out): enqueue adds to the back, dequeue removes from the front. Applications: breadth-first search, task scheduling, print spoolers, and message brokers. Python's collections.deque provides O(1) operations at both ends. A priority queue serves the highest-priority item first and is implemented as a heap — the foundation of Dijkstra's shortest-path algorithm and operating system schedulers.\n\nA hash table stores key-value pairs with O(1) average-case insertion, lookup, and deletion. A hash function maps the key to a bucket index. Collisions are resolved by chaining (a linked list per bucket) or open addressing (probing for the next empty slot). The load factor (n / capacity) controls performance — most implementations resize when it exceeds 0.7–0.75. Python dicts and JavaScript objects are hash table implementations.\n\nChoosing the right structure often determines the difference between an O(n²) and an O(n) solution. Replacing nested loops with hash table lookups is one of the highest-value algorithmic optimizations available in everyday code."),
                ("Sorting and Searching Algorithms", "Sorting and searching are among the most studied problems in computer science. Mastering core algorithms builds intuition for algorithm design, complexity analysis, and technical interview performance.\n\nComparison-based sorting: Bubble sort O(n²) is simple but impractical for large inputs. Insertion sort O(n²) is efficient for nearly-sorted data and small n — used as the base case in hybrid sorts. Merge sort O(n log n) divides, recursively sorts, and merges — stable, predictable, but requires O(n) extra space. Quick sort O(n log n) average partitions around a pivot — in-place, very fast in practice but O(n²) worst case with poor pivot choice. Timsort (Python and Java's default sort) is a hybrid of merge and insertion sort with excellent real-world performance on partially ordered data.\n\nNon-comparison sorting bypasses the O(n log n) lower bound for comparison sorts by exploiting data properties. Counting sort O(n+k) works for integer data in a known range k. Radix sort processes digits from least to most significant. Bucket sort distributes elements into buckets and sorts each. These achieve O(n) for specific domains.\n\nSearching: linear search O(n) scans sequentially — no prerequisites, works on unsorted data. Binary search O(log n) halves the remaining search space each step — requires sorted input. Hash set membership testing is O(1) average — the most efficient for repeated lookups.\n\nAlgorithm design strategies: divide and conquer (merge sort, binary search, fast Fourier transform), greedy (locally optimal choices at each step), dynamic programming (cache overlapping subproblems — Fibonacci, Levenshtein distance, knapsack), and backtracking (explore then undo — N-queens, Sudoku solver). Recognizing which strategy applies is the core skill for algorithmic problem-solving.")
            }},
            { 9, new List<(string, string)>
            {
                ("Relational Database Concepts", "Relational databases organize data into tables of rows and columns, enforcing structure and relationships through a schema. They are the most widely used database type for transactional and business-critical workloads.\n\nA table represents an entity type. Each row is a record (instance); each column is an attribute with a defined data type and constraints. The primary key uniquely identifies each row. A foreign key in one table references the primary key of another, establishing a relationship between entities and enforcing referential integrity — the database rejects inserts or deletes that would create orphaned references.\n\nThree relationship types: one-to-one (a user has one profile), one-to-many (a customer has many orders), many-to-many (students enroll in many courses, courses have many students — bridged with a junction table). Correctly modelling relationships prevents data duplication and maintains consistency.\n\nACID properties guarantee reliable transactions: Atomicity (all operations in a transaction succeed or all are rolled back), Consistency (only valid data transitions are committed), Isolation (concurrent transactions do not see each other's partial changes), Durability (committed data survives system failures). These properties make relational databases trusted for banking, e-commerce, and healthcare applications.\n\nPopular relational databases: PostgreSQL (open-source, highly standards-compliant, excellent for complex queries), MySQL/MariaDB (widely used in web stacks), SQLite (serverless, embedded, ideal for mobile and development), Microsoft SQL Server, and Oracle. SQL syntax is largely portable across all of them."),
                ("SQL Queries and Joins", "SQL (Structured Query Language) is the standard language for defining, manipulating, and querying relational data. Mastering SELECT, filtering, aggregation, and joins covers the vast majority of day-to-day database work.\n\nBasic SELECT: SELECT column1, column2 FROM table WHERE condition ORDER BY column ASC|DESC LIMIT n. Filter with comparison operators (=, <>, <, >, BETWEEN, IN, LIKE with % wildcard), logical operators (AND, OR, NOT), and NULL checks (IS NULL, IS NOT NULL — never use = NULL).\n\nJoins combine rows from multiple tables: INNER JOIN returns only matched rows from both tables. LEFT JOIN returns all rows from the left table and matching rows from the right (NULL for unmatched). RIGHT JOIN is the mirror. FULL OUTER JOIN returns all rows from both. Self-join queries a table against itself — useful for hierarchical data such as employees and managers in the same table.\n\nAggregation: GROUP BY groups rows sharing a column value; aggregate functions summarize each group: COUNT(*), SUM(col), AVG(col), MIN(col), MAX(col). HAVING filters groups after aggregation (WHERE filters rows before): SELECT department, AVG(salary) FROM employees GROUP BY department HAVING AVG(salary) > 60000.\n\nSubqueries embed a SELECT inside another query: WHERE salary > (SELECT AVG(salary) FROM employees). CTEs (Common Table Expressions) with WITH name intermediate results for readability. Window functions (ROW_NUMBER, RANK, SUM OVER PARTITION BY) perform calculations across a set of rows without collapsing them into a group. Indexes on frequently queried and joined columns are the single most impactful performance improvement in production databases."),
                ("Database Design and Normalization", "Good database design prevents anomalies, reduces redundancy, and ensures data integrity. Normalization is the systematic process of organizing tables to remove duplication and problematic dependencies.\n\nFirst Normal Form (1NF): columns hold atomic values (no lists or repeating groups in a cell), there are no duplicate rows, and each table has a primary key. Second Normal Form (2NF): all non-key attributes depend on the entire primary key, not just part of it — relevant for composite keys. Third Normal Form (3NF): no non-key column depends on another non-key column (transitive dependency removed). Boyce-Codd Normal Form (BCNF) is a stricter version of 3NF.\n\nEntity-Relationship (ER) diagrams visualize entities, their attributes, and relationships before writing any DDL. Spending time on a data model diagram prevents expensive restructuring after the application is in production. Tools: dbdiagram.io, Lucidchart, draw.io.\n\nDenormalization deliberately re-introduces some redundancy for read performance — common in analytical databases and data warehouses. The STAR schema (a central fact table surrounded by dimension tables) optimizes analytical queries by minimizing joins at the cost of some write redundancy.\n\nSQL DDL: CREATE TABLE defines columns with data types and constraints (NOT NULL, UNIQUE, CHECK, DEFAULT, PRIMARY KEY, FOREIGN KEY). ALTER TABLE modifies structure non-destructively. CREATE INDEX adds a B-tree or hash index to speed lookups and joins at the cost of slower writes. DROP TABLE removes a table and its data permanently. Understanding DDL enables both application developers and database administrators to collaborate effectively on schema evolution.")
            }},
            { 10, new List<(string, string)>
            {
                ("Software Development Lifecycle", "The Software Development Lifecycle (SDLC) is a structured process for planning, building, testing, and delivering software systems. Understanding it helps teams produce higher-quality software on more predictable timelines.\n\nCore SDLC phases: Requirements (stakeholders define what the system must do — functional and non-functional), Design (architects plan structure, data models, APIs, and interfaces), Implementation (developers write code following agreed standards), Testing (QA validates functionality, performance, security, and edge cases), Deployment (release to production environments), and Maintenance (bug fixes, enhancements, and monitoring).\n\nWaterfall is a linear model — each phase must complete before the next begins. It works when requirements are stable and well-understood upfront, such as embedded systems or regulatory software. Spiral is iterative with explicit risk analysis at each cycle. Incremental delivers working functionality in successive releases of growing scope.\n\nAgile is an iterative, feedback-driven approach. Teams deliver working software in short cycles (sprints of 1–4 weeks) and adapt plans based on new information and stakeholder feedback. The Agile Manifesto values individuals and interactions over processes, working software over comprehensive documentation, customer collaboration over contract negotiation, and responding to change over following a plan.\n\nSDLC metrics that indicate health: cycle time (idea to production), defect escape rate (bugs reaching users), deployment frequency, and mean time to recovery (MTTR). Measuring these helps teams identify bottlenecks and continuously improve their delivery process."),
                ("Design Patterns", "Design patterns are reusable solutions to recurring software design problems. They capture best practices from experienced developers into a shared vocabulary that improves communication and code quality.\n\nCreational patterns control object creation: Singleton ensures a class has exactly one instance (e.g., a configuration manager or connection pool) with global access. Factory Method delegates instantiation to subclasses, allowing flexible object creation without specifying the exact class. Builder constructs complex objects step by step — used in query builders, HTTP request builders, and test data factories.\n\nStructural patterns organize classes and objects: Adapter wraps an incompatible interface to make it work with existing code — essential when integrating third-party libraries. Decorator attaches additional behavior to an object dynamically without altering its class — Python's @ syntax implements this directly. Facade provides a simplified unified interface to a complex subsystem, hiding its complexity from clients.\n\nBehavioral patterns define communication between objects: Observer establishes a one-to-many dependency so dependents are notified automatically of state changes — the foundation of event systems, reactive frameworks, and publish-subscribe architectures. Strategy encapsulates interchangeable algorithms behind a common interface, enabling runtime switching. Command encapsulates a request as an object, enabling undo/redo, queuing, and logging.\n\nThe Gang of Four book (1994) catalogued 23 foundational patterns. Knowing when to apply a pattern — and equally when to keep things simple — is the mark of experienced engineering. Patterns communicate intent; overapplication creates accidental complexity that slows teams down."),
                ("Agile, Scrum, and Clean Code", "Agile methodologies and clean code practices are dual pillars of modern professional software development — they govern how teams organize work and how individuals write maintainable code.\n\nScrum is the most widely adopted Agile framework. Roles: Product Owner (prioritizes the backlog, represents stakeholders), Scrum Master (facilitates ceremonies, removes impediments), Development Team (self-organizing, cross-functional). Ceremonies: Sprint Planning (select and break down backlog items), Daily Standup (15-minute synchronization on progress and blockers), Sprint Review (demo to stakeholders), Sprint Retrospective (team reflects on process improvements). Artifacts: Product Backlog (all desired work), Sprint Backlog (items committed for the sprint), Increment (potentially shippable product).\n\nKanban is a continuous flow model using columns (To Do, In Progress, Done) and WIP limits to reduce context switching and expose bottlenecks. It suits support teams and operational workflows where items arrive unpredictably.\n\nClean code principles: functions should do one thing (Single Responsibility Principle), names should clearly reveal intent, avoid magic literals (use named constants), keep functions short and focused, and leave code cleaner than you found it (Boy Scout Rule). SOLID principles — Single Responsibility, Open/Closed, Liskov Substitution, Interface Segregation, Dependency Inversion — guide object-oriented design toward maintainable, extensible systems.\n\nTechnical debt is the accumulated cost of shortcuts taken during development that increase future maintenance effort. Regular refactoring — restructuring code without changing its behavior — keeps debt manageable. Teams that neglect code quality slow down progressively as the codebase becomes harder to understand, test, and safely change.")
            }}
        };
